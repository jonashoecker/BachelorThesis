...
def jacknife_structure_factor(samples):
...
def calc(model, temperature, N):
	...
	# apply jacknife analysis
	S, e = jacknife_structure_factor(S_q_samples)
...
def main():
    ...
    temperatures = [1.5, 2.2680, 4]
    model = random_matrix_generation(N, N)

    # write results in a txt file
    with open('structure_factor_data.txt', 'w') as f:
    	f.write('# T qx S_q error\n')

    	for T in temperatures:
    		model_T = model.copy()
    		S_vals, err_vals = calc(model_T, T, N)

    		q_x = [2 * np.pi * nx / N for nx in range(N)]

    		for q, S, err in zip(q_x, S_vals, err_vals):
    			f.write(f'{T:.2f} {q:.2f} {S:.6f} {err:.6f}\n')

    print("Results save in 'structure_factor_data.txt'")
